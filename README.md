...
**当前状态：M0–M4 已实现并可运行**，1147 个测试在无网络、无凭证的机器上通过（含 6 轮对抗评审后的安全回归，与 540 个自动生成的熔断表不变量组合）。第 6 轮之后另做了一次净室验证：重建 wheelhouse、`--no-index` 装进全新 venv，再跑一次真实任务（跑挂测试 → 打补丁 → 重跑转绿 → 引用自己工具输出里的 event id 完成），14 项检查全过。
...
tests/                1147 个测试：golden 场景、攻击表、熔断表不变量
```
